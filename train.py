...
from nets.yolact_training import get_lr_scheduler
...
if __name__ == "__main__":
    ...
    if True:
        ...
        if eager:
            ...
            gen         = tf.data.Dataset.from_generator(partial(train_dataloader.generate), (tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32))
            gen_val     = tf.data.Dataset.from_generator(partial(val_dataloader.generate), (tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32))
            ...
            if ngpus_per_node > 1:
                with strategy.scope():
                    model.compile(loss={'yolact_Loss': lambda y_true, y_pred: y_pred}, optimizer = optimizer)
            else:
                model.compile(loss={'yolact_Loss': lambda y_true, y_pred: y_pred}, optimizer = optimizer)
